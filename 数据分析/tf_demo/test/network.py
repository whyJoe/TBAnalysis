...
model.add(keras.layers.Dense(80, input_shape=(54,), activation='relu'))
...
print(model.summary())
history = model.fit(x, y, epochs=100, batch_size=10, validation_split=0.2, verbose=1)
...
for i in range(0, len(y_pre)):
    if aa[i] == 1 and y[i] == 1:
        tp = tp + 1
    elif aa[i] == 1 and y[i] == 0:
        fp = fp + 1
    elif aa[i] == 0 and y[i] == 0:
        tn = tn + 1
    else:
        fn += 1
